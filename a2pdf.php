...
switch ($pagemode) {
	case 0:
		$html_files = question_per_page($attemptobj, $options);
		break;
	case 1:
		$html_files = questions_paged($attemptobj, $options);
		break;
	case 2:
		# code...
		$html_files = all_questions($attemptobj, $options);
		break;
	default:
		$html_files = question_per_page($attemptobj, $options);
		break;
...
function question_per_page($attemptobj, $options) {
	...
	for ($page=0; $page < $num_pages; $page++) {
		...
		foreach ($questionids as $questionid) {
			...
			$output = render($attemptobj, $slots, $page, $showall, $lastpage, $options);
...
function questions_paged($attemptobj, $options) {
	$tmp_html_files = array();
	$showall = false;
	$num_pages = $attemptobj->get_num_pages();
...
function all_questions($attemptobj, $options) {
...
function render($attemptobj, $slots, $page, $showall, $lastpage, $options) {
	global $PAGE, $CFG;
	...
	$summarydata = summary_table($attemptobj, $options);
 ...
 */
function summary_table($attemptobj, $options) {
	global $USER, $DB;
